import { part1, part2, validateColor, validateEyeColor, validateHeight, validatePid, validateYear } from '../day4'

type EachTestArg = {
  value: string,
  isValid: boolean,
  testFunc: (...args: any[]) => boolean,
  extraArgs: any[]
}
...
describe('day 4', () => {
  ...
  it('part 1 example works', () => {
    const actual = part1(sampleInput)
  ...
  it('part 2 example works', () => {
    const actual = part2(part2Sample)
    expect(actual).toBe(4)
  })

  it('part 2 works', () => {
    const actual = part2(input)
    expect(actual).toBe(194)
  })

  it.each`
    value           | isValid  | testFunc            | extraArgs
    ${'2002'}       | ${true}  | ${validateYear}     | ${[1920, 2002]}
    ${'2003'}       | ${false} | ${validateYear}     | ${[1920, 2002]}
    ${'60in'}       | ${true}  | ${validateHeight}   | ${[]}
    ${'190cm'}      | ${true}  | ${validateHeight}   | ${[]}
    ${'190in'}      | ${false} | ${validateHeight}   | ${[]}
    ${'190'}        | ${false} | ${validateHeight}   | ${[]}
    ${'#123abc'}    | ${true}  | ${validateColor}    | ${[]}
    ${'#123abz'}    | ${false} | ${validateColor}    | ${[]}
    ${'123abc'}     | ${false} | ${validateColor}    | ${[]}
    ${'brn'}        | ${true}  | ${validateEyeColor} | ${[]}
    ${'wat'}        | ${false} | ${validateEyeColor} | ${[]}
    ${'000000001'}  | ${true}  | ${validatePid}      | ${[]}
    ${'0123456789'} | ${false} | ${validatePid}      | ${[]}
  `('validates appropriately: $testFunc.name("$value") => $isValid', ({ value, isValid, testFunc, extraArgs }: EachTestArg) => {
    const args = [value, ...extraArgs]
    expect(testFunc.apply(null, args)).toBe(isValid)
  })
